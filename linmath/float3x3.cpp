...
const float3x3 inverse(const float3x3 &m) {
    float detTrans = determinant(m);
    assert(detTrans != 0);

    float
        A =   m.c2.y*m.c3.z - m.c3.y*m.c2.z,
        B = -(m.c1.y*m.c3.z - m.c3.y*m.c1.z),
        C =   m.c1.y*m.c2.z - m.c2.y*m.c1.z,
        D = -(m.c2.x*m.c3.z - m.c3.x*m.c2.z),
        E =   m.c1.x*m.c3.z - m.c3.x*m.c1.z,
        F = -(m.c1.x*m.c2.z - m.c2.x*m.c1.z),
        G =   m.c2.x*m.c3.y - m.c3.x*m.c2.y,
        H = -(m.c1.x*m.c3.y - m.c3.x*m.c1.y),
        I =   m.c1.x*m.c2.y - m.c2.x*m.c1.y;


    return (1 / detTrans) * make_matrix(A, D, G, B, E, H, C, F, I);
}
